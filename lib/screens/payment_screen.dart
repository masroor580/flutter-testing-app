...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  List.generate(6, (index) => FocusNode()); // Ensure proper disposal
  ...
  void dispose() {
    for (int i = 0; i < _controllers.length; i++) {
      ...
      _rawFocusNodes[i].dispose(); // Dispose RawKeyboardListener focus nodes
    }
    super.dispose();
  }
  ...
  void _onContinuePressed(BuildContext context) {
    bool isComplete = _controllers.every((c) =>
    c.text
        .trim()
        .isNotEmpty);
    if (!isComplete) {
      if (_otpError != "Please enter complete OTP") {
        setState(() {
